...
void search(vector<string> transactionList, string operationP);
...
void printLogOperation(string operation, vector<string> transactionList) {
    ...
    for (int i = 0; i < numThreads; ++i) {
        ...
        cout<<"D> thread joined: "<<threads[i]<<endl;
    }
    cin.ignore();
    cin.get();
}

// Function to print log operation details
void* OperationRunner(void* arg) {
...
void search(vector<string> transactionList, string operationP) {
	...
	for (int i = 0; i < numThreads; i++) {
		...
		threadArgs[i].operation = operationP;
...
void displayTotalDeposited(vector<string> transactionList) {
	//clearScreen();
	totalDepositAmount = 0; //reset total for latest value
	
	
	search(transactionList, "Deposit");
...
void displayTotalWithdrawn(vector<string> transactionList) {
	...
	totalWithdrawalAmount = 0; //reset total for latest value
	
	search(transactionList, "Withdraw");
